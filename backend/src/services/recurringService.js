...
function validateRecurringInput(input) {
  ...
  if (!["daily", "weekly", "monthly", "yearly"].includes(input.cadence || "monthly")) {
    const err = new Error("cadence must be daily, weekly, monthly, or yearly");
    err.status = 400;
    throw err;
  }
  if (!input.start_date) {
...
export async function updateRecurring(id, input) {
  ...
  const sql = `
    ...
    SET name=$1, amount=$2, type=$3, cadence=$4, start_date=$5, end_date=$6, account_id=$7, category_id=$8, active=$9,
        future_amount=$10, future_amount_start_date=$11
    WHERE id=$12
    RETURNING *
  `;
  const params = [
